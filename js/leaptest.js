...
Leap.loop(controllerOptions, function(frame) {
  ...
  var pointableString = "";
  var greatDirection = 0;

  //$(".finger").css("display", "none");
  if (frame.pointables.length > 0) {
    for (var i = 0; i < frame.pointables.length; i++) {
      var pointable = frame.pointables[i];
      //$("#finger" + j).css("display", "block");
      //$("#finger" + j).css("left", pointable.direction[0] * $(window).width());
      //$("#finger" + j).css("top", pointable.direction[1] * $(window).height());
      //$("#finger" + j).css("transform:", "scale("+pointable.tipPosition[3]+","+pointable.tipPosition[3]+")");
    ...
    for (var i = 0; i < frame.gestures.length; i++) {
      ...
      switch (gesture.type) {
        ...
        case "swipe":
        	...
        	//if(gesture.direction[0] > 0.6 && handClose) Salts.largeSwipeHorizontal(gesture.direction[0]);
	    	 // if(gesture.direction[0] < -0.6 && handClose) Salts.largeSwipeHorizontal(gesture.direction[0]);
